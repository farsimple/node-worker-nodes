...
class WorkerNodes extends EventEmitter {
    ...
    dispatchTaskTo(worker) {
        ...
        /**
         * BEGIN CONNIO MODIFICATION
         */
        let timeout = task.args[0].timeout || this.options.taskTimeout;
        if (isFinite(timeout)) {
            task.timer = setTimeout(() => this.handleWorkerTimeout(worker), timeout);
        }
        /**
         * END CONNIO MODIFICATION
         */
    }
